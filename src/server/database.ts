...
export function createDatabase(filename: string): OrchestratorDatabase {
  ...
  if (!hasColumn('agents', 'option_templates_json')) {
    connection.exec("ALTER TABLE agents ADD COLUMN option_templates_json TEXT NOT NULL DEFAULT '{}'");
  }
  if (!hasColumn('agents', 'option_values_json')) {
    connection.exec("ALTER TABLE agents ADD COLUMN option_values_json TEXT NOT NULL DEFAULT '{}'");
  }
  ...
  if (!hasColumn('org_agents', 'organization_id')) {
    connection.exec(
      `ALTER TABLE org_agents ADD COLUMN organization_id TEXT NOT NULL DEFAULT '${DEFAULT_ORGANIZATION_ID}'`,
    );
  }
  ...
  if (!hasColumn('projects', 'organization_id')) {
    connection.exec(
      `ALTER TABLE projects ADD COLUMN organization_id TEXT NOT NULL DEFAULT '${DEFAULT_ORGANIZATION_ID}'`,
    );
  }
